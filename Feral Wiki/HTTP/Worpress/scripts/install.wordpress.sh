...
wordpressconf="https://raw.githubusercontent.com/feralhosting/feralfilehosting/master/Feral%20Wiki/HTTP/Worpress/conf/wordpress.conf"
#
############################
...
############################
#
    if [ ! -d "${feralwww}wordpress" ]
    then
        echo -e "Downloading and extracting latest version to:"
        echo
        echo -e "\033[32m""${host2https}wordpress/""\e[0m"
        echo
        wget -qO "$HOME"/latest.tar.gz "$wordpressurl"
        tar xf "$HOME"/latest.tar.gz -C "${feralwww}"
        # insert user socket path instead of localhost
        sed -i 's|<td><input name="dbhost" id="dbhost" type="text" size="25" value="localhost" /></td>|<td><input name="dbhost" id="dbhost" type="text" size="25" value="<?php echo  '\'':'\'' . getenv('\''HOME'\'') . '\''/private/mysql/socket'\''; ?>" /></td>|g' "${feralwww}wordpress/wp-admin/setup-config.php"
        #
