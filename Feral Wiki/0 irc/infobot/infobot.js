...
infobot.addListener('message', function(from, to, message) {
   ...
   if ( message == '%timezone' ) {
    ...
    if ( message == '%help' ) {
        infobot.say(to, "Available triggers: %activated %newuser %common %rtorrent %deluge %transmission %vpn %mysql %restart %delugethin %changeclient %ssh %xshell %kitty %sshfs %quota %publickey %tunnels %slowftp %nginx %WWW %webapps %xmbc %subsonic %ampache %icecast %upgrade %late %proftpd %autodl %rsync %irssi %ip %timezone");
        return;
    }
