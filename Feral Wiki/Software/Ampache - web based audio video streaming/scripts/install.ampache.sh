...
############################
#
ffmpegfv="http://johnvansickle.com/ffmpeg/releases/ffmpeg-release-64bit-static.tar.xz"
...
#
	...
	wget -qO ~/ffmpeg.tar.xz "$ffmpegfv"
    tar xf ~/ffmpeg.tar.xz --strip-components=1 -C ~/.ampache/ffmpeg
	chmod 700 ~/.ampache/ffmpeg/{ffmpeg,ffmpeg-10bit,ffprobe,ffserver,qt-faststart}
	cd && rm -rf ampache.zip ampache-"$ampacheversion" ffmpeg.tar.xz
